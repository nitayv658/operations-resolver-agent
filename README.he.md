...
*(רשות, עד 2 דקות — הוסיפו כאן קישור שמציג הרצת happy-path והרצת מקרה קצה דרך
`run_ticket.py` או `run_scenarios.py`)*

## הערה על הפיקסצ'רס

המטלה מבקשת לסמן ולא לערוך כל דבר שנראה חריג ב-`starter-kit/data/`. לא נמצא דבר
שדרש סימון — כל 33 הבדיקות ב-`verify_scenarios.py` עוברות מול הפיקסצ'רס כפי
שסופקו.
